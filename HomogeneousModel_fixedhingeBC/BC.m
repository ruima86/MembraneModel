function err = BC(x,p,sigma,C0,R0,Z,Rb,angle)
f = x(1);
dpsi0 = x(2);
psi0 = 0;
...
tspan=[0,50];
termFunc1 = @(t,y) terminate1(t,y,Rb,angle);
options1 = odeset('Events',termFunc1,'AbsTol',1e-16,'RelTol',1e-12);
termFunc2 = @(t,y) terminate2(t,y,Rb,angle);
...
err1 = (Z_end-Z)^2 + (psi(end)-angle)^2 + (R_end-Rb)^2;
...
err2 = (Z_end-Z)^2 + (psi(end)-angle)^2 + (R_end-Rb)^2;
